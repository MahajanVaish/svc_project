@forelse($inquiries as $inquiry)
    <tr>
        ...
        <td class="py-2.5 px-3">
            <a href="{{ route('diet.join.patient', ['id' => $inquiry->id, 'redirect_to' => 'diet_chart']) }}" style="color: #28a745; text-decoration: none;"
                title="View/Edit Diet Chart">
                Diet H/O
            </a>
        </td>
        <td class="text-center py-2.5 px-3">
            <button type="button" onclick="reverseToFollowup({{ $inquiry->id }}, 'diet_chart', '{{ addslashes($inquiry->patient_name ?? ($inquiry->patient_f_name ?? 'this patient')) }}')" class="action-btn"
